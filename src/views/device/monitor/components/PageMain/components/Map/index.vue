<template>
    <div id="antv-l7-map" style="position: relative; width: 100%; height: 452px" />
</template>
<script>
import { Scene, LineLayer, PointLayer, PolygonLayer, Popup } from '@antv/l7'
import { Mapbox } from '@antv/l7-maps'
// import world from '@/assets/map/map01.json'
// import dot from '@/assets/map/map02.json'
// import flyLine from '@/assets/map/map03.json'
export default {
    mounted() {
        this.initMap()
        this.addLayer()
    ...
    methods: {
        ...
        addLayer() {
            Promise.all([
                ...
                // fetch('https://gw.alipayobjects.com/os/basement_prod/dbd008f1-9189-461c-88aa-569357ffc07d.json').then(d => d.json()), // 使用这个地图数据添加图层速度很慢，暂时不知道怎么解决
                ...
                const flydata = eval(flyLine).map(item => {
                    const latLng1 = item.from.split(',').map(e => e * 1)
                    const latLng2 = item.to.split(',').map(e => e * 1)
                ...
                const flyLineLayer = new LineLayer({blend: 'normal'})
                    ...
                    .shape('arc3d')
                    .size(2.6)
                    .animate({ interval: 2, trailLength: 2, duration: 1 })
                    .style({opacity: 1})
                ...
                //     const popup = new Popup({ offsets: [0, 0], closeButton: false })
                //     .setLnglat(e.lngLat)
                //     .setHTML(`地理可视化引擎 AntV L7:  <a  target='_blank', href='https://github.com/antvis/L7'>GitHub</a>`)
