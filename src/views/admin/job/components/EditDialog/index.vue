<template>
    <el-dialog :append-to-body="true" :close-on-click-modal="false" :before-close="cancel" :visible.sync="dialog" :title="isAdd ? '新增岗位' : '编辑岗位'" width="500px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
            ...
            <el-form-item style="margin-bottom: 0px;" label="所属部门" prop="dept">
                <treeselect v-model="form.dept" :options="depts" style="width: 370px;" placeholder="选择所属部门" />
            </el-form-item>
...
export default {
    ...
    data() {
        return {
            ...
            dialog: false, 
            loading: false, 
            form: { id: null, name: null, sort: 999, state: 'on', dept: null },
            rules: {
                name: [
                ...
                sort: [
                    { required: true, message: '请填写岗位排序', trigger: 'blur' }
                ]
            },
        }
    },
    methods: {
        ...
        async doAdd() {
            let result = await api.job_add(this.form)
        ...
        async doEdit() {
            let result = await api.job_edit(this.form) 
            this.loading = false // 隐藏菊花
            if (result.valid) {
                this.$notify({
                    title: '修改成功',
        ...
        resetForm() {
            ...
            this.form = { id: null, name: null, sort: 999, state: 'on', dept: null }
        },
    }
}
</script>
