...
;(async function() {
    ...
    async function write() {
        ...
        scanResult.forEach(file => {
            for (const name in file) {
                if (file.hasOwnProperty(name)) {
                    const { scope, prefix, body, description } = file[name]
                    data[name] = {
                        scope,
                        prefix: `dd ${prefix}`,
                        body,
        ...
        await fs.writeFileSync(
            path.join(resolve('../.vscode'), 'd2.code-snippets'),
    ...
    fs.watch(
        ...
        (eventType, filename) => {
            if (filename) {
                if (
                    snippetJsonFileNameExp.test(filename) ||
                    snippetJsFileNameExp.test(filename)
                ) {
